...
test.describe('RTMS auth and role routing', () => {
  test('admin can log in and is routed to admin dashboard', async ({ page }) => {
  ...
  test('tenant can log in and is routed to tenant home', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'My Unit' })).toBeVisible();

    await page.goto('/admin/dashboard');
    await expect(page).toHaveURL(/\/tenant\/home$/);
  });

  test('invalid credentials stay on login with validation error', async ({ page }) => {
    await page.goto('/login');
