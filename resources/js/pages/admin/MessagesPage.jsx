...
export default function MessagesPage({ messages = [] }) {
    ...
    const threads = useMemo(() => {
        ...
        for (const message of messages) {
            ...
            const unreadDelta =
                message.from === 'tenant' && !message.read ? 1 : 0;
    ...
    return (
        <div className="grid h-[calc(100vh-120px)] grid-cols-12 gap-5">
                    ...
                    {threads.length === 0 ? (
                        ...
                        threads.map((thread) => (
                            <button
                                key={thread.tenantId}
                                className={[
                                    'w-full border-b border-[#1B2B4B]/6 px-4 py-3 text-left transition-colors',
            ...
            <Card className="col-span-8 flex flex-col overflow-hidden">
                ...
                <Card.Body className="flex-1 space-y-2 overflow-y-auto">
                    {selectedThread.length === 0 ? (
                        <div className="text-sm text-[#5C6B88]">
                            Select a tenant thread to view messages.
                        </div>
                    ) : (
                        selectedThread.map((message) => (
                            ...
                            >
                                ...
                                >
                                    <p>{message.text}</p>
                                    <p
                                        className={`mt-1 text-[10px] ${message.from === 'admin' ? 'text-white/70' : 'text-[#5C6B88]'}`}
                                    >
                                        {formatDateDisplay(
                                            message.created_at,
                                            '-',
